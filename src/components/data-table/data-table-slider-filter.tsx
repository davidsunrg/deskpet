...
export function DataTableSliderFilter<TData>({ column, title, }: DataTableSliderFilterProps<TData>) {
    ...
    const { min, max, step } = React.useMemo<Range & {
        step: number;
    }>(() => {
        let minValue = 0;
        let maxValue = 100;
    ...
    const onSliderValueChange = React.useCallback((value: number | readonly number[]) => {
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              ...
              {unit && (<span className="absolute top-0 right-0 bottom-0 flex items-center rounded-r-md bg-accent px-2 text-muted-foreground text-sm">
                  {unit}
                </span>)}
            </div>
