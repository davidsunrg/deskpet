...
export function PetCardGrid({
  ...
  const [adoptPet, setAdoptPet] = useState<ShowcasePet | null>(null);
  ...
  const openAdopt = (pet: ShowcasePet) => {
    ...
    <>
      ...
      {adoptPet ? (
        <PetProfileDialog
          ...
          open
          onOpenChange={(open) => {
            if (!open) setAdoptPet(null);
          }}
          presetKey={adoptPet.id}
          defaults={{
            name: '',
            species: adoptPet.species,
            breed: adoptPet.breed,
