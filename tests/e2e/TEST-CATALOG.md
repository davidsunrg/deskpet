...
| 4 | DeskPet footer renders reference layout | Open `/`, verify footer shows DeskPet brand, tagline, link columns, and centered copyright with rights reserved text. |
| 5 | Open mobile navigation hides at desktop breakpoint | Open the mobile navigation at a narrow viewport, resize to desktop width, and verify the mobile dialog no longer covers the desktop navigation. |
| 6 | Mobile navigation uses consistent interactive backgrounds | At a narrow viewport, verify standalone links gain the muted background on hover and active standalone, group, and nested links retain the same background treatment. |
